# Calculate average coefficients for sensitivity analysis of survival
# Marco Andrello
...
names_var <- c("surv") # Only survival
...
theta       <- list()
r_squared   <- list()
load(paste0(getwd(),"/../../noBackup/Arabis/ResultsGLMM/SensSurv/Models_Clim1_Boot1.RData"))
...
for (i.clim in startclim : endclim){
  for(i.boot in 1 : nboot) {
    ...
    load(paste0(getwd(),"/../../noBackup/Arabis/ResultsGLMM/SensSurv/Models_Clim",i.clim,"_Boot",i.boot,".RData"))
    # Loop on variables
    for (i.var in 1 : length(names_var)) {
      var <- names_var[i.var]
      d.var <- paste0("d.",var)
...
save(coef_fixed, sd_rand, theta, r_squared, file="Coefficients_SensSurv.RData")

...
for (i.boot in start.boot : n.boot) {
  ...
  data_boot_growth_scaled$SiteYear <- factor(data_boot_growth_scaled$SiteYear)
  # Fit
  ...
  # Analysis of reproduction (fruiting)
  # Select only necessary columns and filter out NA
  data_boot_fruiting <-
    data_boot %>%
  ...
  data_boot_fruiting_scaled$SiteYear <- factor(data_boot_fruiting_scaled$SiteYear)
  data_boot_fruiting_scaled$SiteQuad <- factor(data_boot_fruiting_scaled$SiteQuad)
  # Fit
  m <- glmer(Fru ~ Nb_Tot.x + Nb_Tot.x.sq + T.mean + T.mean.sq + T.range + Axis1 + Axis2 + (1| Site) + (1 | SiteYear) + (1 + Nb_Tot.x | SiteQuad), family=binomial(),
  ...
  # Analysis of reproductive output (fruits)
  # Select only necessary columns and filter out NA
  data_boot_fruits <-
    data_boot %>%
  ...
  data_boot_fruits_scaled$SiteYear <- factor(data_boot_fruits_scaled$SiteYear)
  # Fit
  ...
  data_boot_recruit_scaled$SiteYear <- factor(data_boot_recruit_scaled$SiteYear)
  # Fit

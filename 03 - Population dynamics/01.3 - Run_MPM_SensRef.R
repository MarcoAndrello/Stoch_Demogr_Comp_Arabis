...
for (i.clim in startclim : endclim) { # Loop on clim
  ...
  for (i.boot in 1 : nboot) {
    ...
    # Load models
    load(paste0("C:/Users/mandrell/Desktop/ARABIS/ResultsGLMM/Models_Clim",i.clim,"_Boot",i.boot,".RData")) 
    ...
    # End loop on sites
    # SLTRE
    res.calc.SLTRE       <- calc.SLTRE_SensRef(matrices.G, matrices.P, pfruit.all, log.nfruits.all, nsd, reduce.matr=TRUE, scen=scen)
...
save(nrepl, y,
     ...
     cnet, log.lambda.R,
     file=paste0("Results_MPM_SensRef_scen_",scen,"_parallel",iparallel,".RData"))
# 
